test_that("identity matrices are possible with a certified PSD witness matrix", {
  for (p in 2:6) {
    res <- check_corr_psd(diag(p), decimals = 2)
    expect_equal(res$verdict, "possible", info = paste("p =", p))
    expect_false(is.null(res$certified_matrix))
  }
})

test_that("a mild well-conditioned matrix is possible", {
  ...
  expect_true(psdness:::.in_box(res$certified_matrix, R, res$delta))
  expect_true(psdness:::.verify_psd(res$certified_matrix))
})

test_that("possible verdicts never carry a negative b_upper", {
...
test_that("certificate() accessor returns the expected shape", {
  ...
  expect_equal(cert$verdict, "impossible")
  expect_length(cert$witness, 3)
  # on a possible case the witness is NULL

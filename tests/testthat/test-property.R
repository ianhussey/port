...
# NEVER return "inconsistent". Because the witness bound is a sound over-estimate,
# this is guaranteed in exact arithmetic; the test guards against an
# over-firing floating-point error model.

test_that("genuine PSD matrices rounded to 2dp are never inconsistent", {
  set.seed(2024)
  n_trials <- n_reps(1500)
  seen <- c(inconsistent = 0L, consistent = 0L, undecided = 0L)
...
test_that("rounding-induced near-boundary cases are handled without false inconsistency", {

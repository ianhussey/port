...
test_that("a genuinely PSD rounded matrix yields verdict 'none'", {
  expect_equal(localize_psd_fault(diag(4), decimals = 2)$localization_verdict, "none")
  set.seed(7)
  L <- matrix(rnorm(20), 5, 4); S <- L %*% t(L); d <- sqrt(diag(S))
...
test_that("severity is always reported and flags egregious violations", {
  ...
  expect_equal(lf$severity$witness_margin, check_corr_psd(triad_R, decimals = 2)$margin)
})

test_that("print.psd_fault narrates the inference", {
  expect_output(print(localize_psd_fault(cell_R, decimals = 2)), "Attributable to a single cell")
  expect_output(print(localize_psd_fault(triad_R, decimals = 2)), "any one of the three")
...
test_that("fault_evidence returns the A-D evidence", {
  ...
  expect_named(ev, c("sole_culprit_cells", "inconsistent_triples",

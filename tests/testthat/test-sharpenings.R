...
test_that("verdicts carry an explicit certification flag", {
  ...
  expect_true(imp$certified)                       # impossibility is a certificate
  ...
  # certified possible <=> a certificate matrix is attached
...
test_that("excusable_delta unifies verdict and severity in precision units", {
  ...
  # verdict consistency: impossible at delta exactly when delta < w
  expect_equal(check_corr_psd(triad_R, delta = w / 2)$verdict, "impossible")
...
test_that(".excusable_decimals translates widths to decimal places", {
  expect_equal(psdness:::.excusable_decimals(0.04), 1)   # 1dp (+-0.05) excuses
  expect_equal(psdness:::.excusable_decimals(0.004), 2)  # 2dp (+-0.005) excuses
  expect_lt(psdness:::.excusable_decimals(0.6), 1)       # nothing conventional does
})
...
test_that(".reported_box builds the correct interval per rounding rule", {
  ...
  near <- psdness:::.reported_box(R, decimals = 2, delta = NULL, "nearest")
  expect_equal(near$lo_raw[1, 2], 0.32 - 0.005)
  expect_equal(near$hi_raw[1, 2], 0.32 + 0.005)
  flo <- psdness:::.reported_box(R, decimals = 2, delta = NULL, "floor")
  expect_equal(flo$lo_raw[1, 2], 0.32); expect_equal(flo$hi_raw[1, 2], 0.33)
  cei <- psdness:::.reported_box(R, decimals = 2, delta = NULL, "ceiling")
  expect_equal(cei$lo_raw[1, 2], 0.31); expect_equal(cei$hi_raw[1, 2], 0.32)
  tru <- psdness:::.reported_box(R, decimals = 2, delta = NULL, "truncate")
...
test_that("matched asymmetric rounding is sound (never impossible on truth)", {
  set.seed(31)
  ...
  for (t in seq_len(150)) {
    ...
    expect_false(identical(
      check_corr_psd(Rf, decimals = 2, rounding = "floor")$verdict, "impossible"))
    Rt <- S; Rt[off] <- dirtrunc(S[off], 2); Rt <- (Rt + t(Rt)) / 2; diag(Rt) <- 1
    expect_false(identical(
      check_corr_psd(Rt, decimals = 2, rounding = "truncate")$verdict, "impossible"))
  }
})
...
test_that("a decimals matrix supports mixed-precision tables soundly", {
  ...
  for (t in seq_len(100)) {
    ...
    expect_false(identical(
      check_corr_psd(Rr, decimals = dec)$verdict, "impossible"))
  }
})

test_that("a gross violation stays impossible under per-cell precision", {
  dec <- matrix(2L, 3, 3)
...
test_that("freeing an NA cell can rescue a triad (verdict for all values)", {
  ...
  expect_equal(res$verdict, "possible")            # some value of (1,2) works
  expect_true(res$certified)
...
test_that("an NA cell does not weaken an independent violation", {
  ...
  expect_equal(res$verdict, "impossible")          # triangle 2 still impossible
...
test_that("the box-sound critical reliability is <= the point closed form", {
  ...
  thr <- res$thresholds
  expect_true(is.finite(thr$rho_impossible_box))
  # rounding gives the box extra slack, so the certified boundary sits at or
  # below the exact-value boundary
  expect_lte(thr$rho_impossible_box, thr$rho_impossible + 1e-3)
  # and impossibility genuinely holds just below the box-sound boundary
  below <- thr$rho_impossible_box - 0.02
  expect_equal(check_disattenuated_psd(R3, reliability = below,
                                       decimals = 2)$verdict, "impossible")
})
...
test_that("box-sound rho* tightens toward the point value as rounding vanishes", {
  ...
  expect_equal(res$thresholds$rho_impossible_box, 0.5, tolerance = 5e-3)
})
...
test_that("headroom is anchored on the box-sound boundary", {
  ...
  thr <- res$thresholds
  expect_equal(res$headroom, 0.95 - thr$rho_impossible_box, tolerance = 1e-9)
})

# ---- strengthened witness (item 7): soundness must be untouched -----------------

test_that("residual directions + polishing never create false impossibles", {
  set.seed(77)
  for (t in seq_len(300)) {
    S <- rand_psd_corr(sample(3:8, 1))
    expect_false(identical(
      check_corr_psd(round(S, 2), decimals = 2)$verdict, "impossible"))
  }
})

test_that("known impossibles are still detected after strengthening", {
  expect_equal(check_corr_psd(triad_R, decimals = 2)$verdict, "impossible")
  R5 <- matrix(0.8, 5, 5); diag(R5) <- 1
  R5[1, 2] <- R5[2, 1] <- -0.8
  expect_equal(check_corr_psd(R5, decimals = 2)$verdict, "impossible")
})

# ---- claim-object reporting (item 1) ---------------------------------------------

test_that("impossible verdicts surface the benign-generator caution", {

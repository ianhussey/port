...
test_that("excusable_delta unifies verdict and severity in precision units", {
  ...
  expect_false(identical(
    check_corr_psd(triad_R, delta = w * 1.5)$verdict,
    "inconsistent"
  ))
...
test_that("psd_fault severity carries excusable_delta = delta + severity_max", {
  ...
  expect_equal(lf$severity$excusable_delta, lf$delta + lf$severity$severity_max)
  expect_equal(
    lf$severity$excusable_delta,
    excusable_delta(triad_R),
    tolerance = 2e-3
  )
...
test_that(".reported_box builds the correct interval per rounding rule", {
  ...
  expect_equal(flo$lo_raw[1, 2], 0.32)
  expect_equal(flo$hi_raw[1, 2], 0.33)
  cei <- port:::.reported_box(R, decimals = 2, delta = NULL, "ceiling")
  expect_equal(cei$lo_raw[1, 2], 0.31)
  expect_equal(cei$hi_raw[1, 2], 0.32)
  tru <- port:::.reported_box(R, decimals = 2, delta = NULL, "truncate")
  expect_equal(tru$lo_raw[1, 2], 0.32)
  expect_equal(tru$hi_raw[1, 2], 0.33)
  expect_equal(tru$lo_raw[1, 3], -0.46)
  expect_equal(tru$hi_raw[1, 3], -0.45)
  expect_equal(tru$lo_raw[2, 3], -0.01)
  expect_equal(tru$hi_raw[2, 3], 0.01)
})
...
test_that("matched asymmetric rounding is sound (never inconsistent on truth)", {
  ...
  for (t in seq_len(n_reps(150))) {
    ...
    Rf <- S
    Rf[off] <- dirfloor(S[off], 2)
    Rf <- (Rf + t(Rf)) / 2
    diag(Rf) <- 1
    expect_false(identical(
      check_corr_psd(Rf, decimals = 2, rounding = "floor")$verdict,
      "inconsistent"
    ))
    Rt <- S
    Rt[off] <- dirtrunc(S[off], 2)
    Rt <- (Rt + t(Rt)) / 2
    diag(Rt) <- 1
    expect_false(identical(
      check_corr_psd(Rt, decimals = 2, rounding = "truncate")$verdict,
      "inconsistent"
    ))
  }
})
...
test_that("delta with non-nearest rounding errors clearly", {
  expect_error(
    check_corr_psd(diag(3), delta = 0.01, rounding = "floor"),
    "decimals"
  )
})
...
test_that("a decimals matrix supports mixed-precision tables soundly", {
  ...
  for (t in seq_len(n_reps(100))) {
    ...
    expect_false(identical(
      check_corr_psd(Rr, decimals = dec)$verdict,
      "inconsistent"
    ))
  }
})
...
test_that("invalid decimals / delta matrices error", {
  ...
  expect_error(
    check_corr_psd(diag(3), delta = matrix(-0.1, 3, 3)),
    "non-negative"
  )
})
...
test_that("the box-sound critical reliability is <= the point closed form", {
  ...
  expect_equal(
    check_disattenuated_psd(R3, reliability = below, decimals = 2)$verdict,
    "inconsistent"
  )
})
...
test_that("residual directions + polishing never create false inconsistents", {
  ...
  for (t in seq_len(n_reps(300))) {
    ...
    expect_false(identical(
      check_corr_psd(round(S, 2), decimals = 2)$verdict,
      "inconsistent"
    ))
  }
})
...
test_that("inconsistent verdicts surface the benign-generator caution", {
  expect_output(
    print(check_corr_psd(triad_R, decimals = 2)),
    "pairwise deletion"
  )
  expect_output(print(localize_psd_fault(triad_R, decimals = 2)), "polychoric")
})

...
test_that("component B is SOUND: never flags a consistent triple", {
  ...
  for (t in seq_len(n_reps(3000))) {
    ...
    G <- expand.grid(gr)
    gmax <- max(
      1 + 2 * G[, 1] * G[, 2] * G[, 3] - G[, 1]^2 - G[, 2]^2 - G[, 3]^2
    )
...
test_that("component A finds all three sole culprits in a triad", {
  ...
  expect_true(all(vapply(
    sole,
    function(c) abs(c$required_edit) > 0.005,
    logical(1)
  )))
})
...
test_that("severity_max equals the smallest uniform box-widening for feasibility", {
  ...
  expect_equal(
    port:::.box_feasible(bnd_ok$lo, bnd_ok$hi, bnd_ok$off)$status,
    "feasible"
  )
  expect_equal(
    port:::.box_feasible(bnd_no$lo, bnd_no$hi, bnd_no$off)$status,
    "infeasible"
  )
})
...
test_that("localize batch summarizes verdict classes into a tibble", {
  ...
  out <- suppressMessages(localize_psd_fault_batch(
    mats,
    decimals = 2,
    quiet = TRUE
  ))
  ...
  expect_true(all(
    c("implicated", "severity_max", "severity_frob") %in% names(out)
  ))
...
test_that("localize batch logs a verdict-class summary message", {
  expect_message(
    localize_psd_fault_batch(list(triad_R), decimals = 2),

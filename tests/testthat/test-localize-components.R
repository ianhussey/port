...
test_that("component B (impossible_triples) flags the classic triangle", {
  it <- impossible_triples(triad_R, decimals = 2)
...
test_that("component B is SOUND: never flags a possible triple", {
  ...
  for (t in seq_len(3000)) {
    ...
    ub <- psdness:::.triple_det_ub(boxes[[1]], boxes[[2]], boxes[[3]])
    # dense-grid true max
...
test_that("impossible_triples is empty for a genuinely PSD matrix", {
  set.seed(3)
  L <- matrix(rnorm(24), 6, 4); S <- L %*% t(L); d <- sqrt(diag(S))
  S <- S / outer(d, d); diag(S) <- 1
  expect_equal(nrow(impossible_triples(round(S, 2), decimals = 2)), 0L)
})

test_that("component A finds all three sole culprits in a triad", {
  ci <- psdness:::.cell_intervals(triad_R, 0.005, verify = TRUE)
...
test_that("component C (leave-one-out) restores a triangle by removing any vertex", {
  lofo <- psdness:::.lofo_restoring(triad_R, 0.005)
...
test_that("severity_max equals the smallest uniform box-widening for feasibility", {
  ...
  eps <- psdness:::.severity_max(R, 0.005)
  # widening by eps + tiny slack is feasible; widening by eps - slack is not
  bnd_ok <- psdness:::.box_bounds(R, 0.005 + eps + 1e-3)
  bnd_no <- psdness:::.box_bounds(R, 0.005 + eps - 1e-3)
  expect_equal(psdness:::.box_feasible(bnd_ok$lo, bnd_ok$hi, bnd_ok$off)$status, "feasible")
  expect_equal(psdness:::.box_feasible(bnd_no$lo, bnd_no$hi, bnd_no$off)$status, "infeasible")
})
...
test_that("rigor: sole-culprit 'empty' claims are witness-certified under verify=TRUE", {
  ...
  ci <- psdness:::.cell_intervals(J, 0.005, verify = TRUE)

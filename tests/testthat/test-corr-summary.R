...
R3 <- matrix(c(1, 0.2, 0.4, 0.2, 1, 0.6, 0.4, 0.6, 1), 3, 3)
offd <- c(0.2, 0.4, 0.6)
...
test_that("SD is NA with a single correlation; degenerate matrices summarise", {
  ...
  expect_equal(two$r_min, 0.5)
  expect_equal(two$r_max, 0.5)
  expect_equal(two$r_mean, 0.5)
  expect_true(is.na(two$r_sd))
...
test_that("error rows leave the summary as NA", {
  out <- check_corr_psd_batch(
    list(ok = R3, broken = matrix(1, 2, 3)),
    quiet = TRUE
  )

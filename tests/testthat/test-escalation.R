...
# when the cheap constructions did not settle possibility. It is self-contained
...
# matrix, so it resolves to a *certified* possible.
ambiguous_R <- matrix(c(1.00,  0.96,  0.23, -0.21,
                        0.96,  1.00,  0.03, -0.04,
                        0.23,  0.03,  1.00,  0.26,
                       -0.21, -0.04,  0.26,  1.00), 4, 4, byrow = TRUE)
...
test_that("near-boundary matrix reaches the escalation branch", {
  ...
  expect_false(identical(res$verdict, "impossible"))  # witness did not fire
})

test_that("POCS resolves the ambiguous case to a certified possible", {
  res <- check_corr_psd(ambiguous_R, decimals = 2)
  ...
  expect_true(psdness:::.in_box(res$certified_matrix, ambiguous_R, res$delta))
  expect_true(psdness:::.verify_psd(res$certified_matrix))
})
...
test_that("a genuinely boundary-only case is honestly undecided, not guessed", {
  ...
  expect_equal(res$tier, "pocs")
  expect_false(identical(res$verdict, "impossible"))  # never a false impossible
})

test_that(".pocs_possible returns a verified in-box matrix when one exists", {
  # exactly-singular (rank-2) PSD matrix: box contains a PSD matrix
  L <- matrix(c(1, 0, 0.6, 0.8, 0.6, -0.8), 3, 2, byrow = TRUE)
  Rs <- L %*% t(L); diag(Rs) <- 1
  hit <- psdness:::.pocs_possible(Rs, 0.005)
  expect_false(is.null(hit))
  expect_true(psdness:::.in_box(hit$X, Rs, 0.005))
  expect_true(psdness:::.verify_psd(hit$X))
  # and the top-level verdict is never impossible
  expect_false(identical(check_corr_psd(Rs, decimals = 2)$verdict, "impossible"))
})

...
.validate_corr <- function(R,
                           sym_warn = 1e-8,
                           sym_error = 1e-4,
                           diag_tol = 1e-8) {
  if (is.data.frame(R)) R <- as.matrix(R)
  if (!is.matrix(R) || !is.numeric(R)) {
    stop("`R` must be a numeric matrix (or a data frame coercible to one).",
         call. = FALSE)
  }
  if (nrow(R) != ncol(R)) {
    stop(sprintf("`R` must be square; got %d x %d.", nrow(R), ncol(R)),
         call. = FALSE)
  }
  p <- nrow(R)
  if (p < 2L) {
  ...
  if (anyNA(R) || any(!is.finite(R))) {
    stop("`R` contains NA or non-finite entries; please supply a complete, ",
         "finite matrix.", call. = FALSE)
  }
  ...
  if (asym > sym_error) {
    stop(sprintf(
      paste0("`R` is not symmetric (max |R - t(R)| = %.3g > %.3g). ",
             "Refusing to symmetrize a strongly asymmetric matrix."),
      asym, sym_error), call. = FALSE)
  }
  if (asym > sym_warn) {
    warning(sprintf(
      paste0("`R` is not exactly symmetric (max |R - t(R)| = %.3g); ",

...
print.disattenuation_check <- function(x, digits = 2, ...) {
  ...
  bind_txt <- function(which_bind) {
    if (identical(which_bind, "PSD")) {
      "loses positive semidefiniteness"
    } else {
      "produces a corrected correlation above 1"
    }
  }
  # Box-sound boundary when available; the point closed form as reference.
  rho_box <- thr$rho_inconsistent_box
  rho_hdl <- if (is.finite(rho_box %||% NA_real_) && (rho_box %||% 0) > 0) {
    rho_box
  } else {
  ...
  box_txt <- if (identical(rho_hdl, rho_box)) {
    sprintf(
      " (rounding-aware; boundary at exact reported values: %.*f)",
      digits,
      thr$rho_inconsistent
    )
  } else {
  ...
  if (identical(x$mode, "critical")) {
    cat(sprintf(
      paste0(
        "  Critical reliability: the disattenuated matrix is valid ",
        "only for reliability >= %.*f%s (it %s below that).\n"
      ),
      digits,
      rho_hdl,
      box_txt,
      bind_txt(thr$inconsistent_binds)
    ))
    if (cut < 1) {
      cat(sprintf(
        paste0(
          "  It is also *plausible* (all corrected |r| <= %.2f) ",
    ...
    if (!is.null(fl)) {
      if (fl < rho_hdl) {
        cat(sprintf(
          paste0(
            "  Taking %.*f as the lowest reliability these measures ",
        ...
        cat(sprintf(
          paste0(
            "  Even the lowest reliability these measures plausibly ",
  ...
  rel_txt <- if (x$mode == "common") {
    sprintf("reliability = %.2f", x$reliability)
  } else {
    sprintf("per-variable reliabilities (min %.2f)", min(x$reliability))
  }
  cat(sprintf("  Disattenuated (%s): %s.\n", rel_txt, toupper(x$verdict)))
  cat(sprintf(
    "    Largest corrected correlation (at reported values): %.2f.\n",
    x$max_disattenuated
  ))
  ...
  if (identical(x$verdict, "inconsistent")) {
    if (fwd$range_inconsistent) {
      ...
      cat(
        "    A corrected correlation exceeds 1: ",
    ...
    if (fwd$psd_inconsistent) {
      cat(
        "    The disattenuated matrix is not positive semidefinite ",
    ...
    cat(sprintf(
      "    Valid, but a corrected correlation exceeds the plausibility cutoff %.2f: %s.\n",
      cut,
      paste(cells, collapse = "; ")
    ))
  }
  ...
  cat(sprintf(
    paste0("    Valid only if reliability >= %.*f%s (it %s below that)"),
    digits,
    rho_hdl,
    box_txt,
    bind_txt(thr$inconsistent_binds)
  ...
  if (x$mode == "common" && is.finite(x$headroom)) {
    if (x$headroom < 0) {
      cat(sprintf(
        "    The reported %.2f falls %.2f short of the inconsistency boundary.\n",
        x$reliability,
        -x$headroom
      ))
    } else {
      cat(sprintf(
        "    The reported %.2f clears the inconsistency boundary by %.2f.\n",
        x$reliability,
        x$headroom
      ))
    }
  ...
  if (x$mode == "per_variable") {
    cat(
      "    (Thresholds shown are for the common-reliability model; ",
